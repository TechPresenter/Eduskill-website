...
function theme_tokens(): array
{
    return [
        ...
        'colors' => [
            ...
            'tokens' => [
                'color.primary'   => ['label' => 'Primary',    'type' => 'color', 'default' => '#0B4E3D', 'var' => '--primary,--brand,--brand-600'],
                'color.secondary' => ['label' => 'Secondary',  'type' => 'color', 'default' => '#174D3D', 'var' => '--secondary,--brand-700,--brand-2,--info'],
                'color.accent'    => ['label' => 'Accent / CTA', 'type' => 'color', 'default' => '#F15A24', 'var' => '--accent,--orange,--warning'],
                // Warm highlight pair. Yellow carries optimism behind headline
                // words and section washes; gold is its readable sibling for
                // strokes, icons and small text where the pale yellow fails contrast.
                'color.yellow'    => ['label' => 'Highlight (yellow)', 'type' => 'color', 'default' => '#FFE987', 'var' => '--yellow,--highlight'],
                'color.gold'      => ['label' => 'Gold',      'type' => 'color', 'default' => '#E8C52E', 'var' => '--gold'],
                'color.success'   => ['label' => 'Success',    'type' => 'color', 'default' => '#2F8065', 'var' => '--success,--green'],
                'color.success_dark' => ['label' => 'Success (dark)', 'type' => 'color', 'default' => '#1F5C48', 'var' => '--success-dark,--green-dark'],
                ...
                // The warm off-white in the brand palette. design-system.css has
                // seven components reading var(--ivory) — the impact bar, the
                // accent mission card, focus-card media wells, the featured
                // story band, gallery frames, the portal roles panel and the
                // table empty state — and until this entry existed the token was
                // declared nowhere, so all seven were stuck on their inline
                // fallback and no admin could retheme them.
                'color.ivory'     => ['label' => 'Ivory (warm surface)', 'type' => 'color', 'default' => '#FEFEF1', 'scope' => 'site', 'var' => '--ivory'],
                'color.text'      => ['label' => 'Heading text','type' => 'color', 'default' => '#0B4E3D', 'var' => '--text'],
                ...
                'color.sidebar'   => ['label' => 'Admin sidebar', 'type' => 'color', 'default' => '#0B4E3D', 'scope' => 'admin', 'var' => '--sb-bg'],
                'color.sidebar_2' => ['label' => 'Sidebar gradient end', 'type' => 'color', 'default' => '#0B3A2E', 'scope' => 'admin', 'var' => '--sb-bg-2'],
                'color.sidebar_active' => ['label' => 'Sidebar active', 'type' => 'color', 'default' => '#174D3D', 'scope' => 'admin', 'var' => '--sb-active'],
                'color.footer'    => ['label' => 'Footer base', 'type' => 'color', 'default' => '#0B4E3D', 'scope' => 'site', 'var' => '--footer-bg'],
            ],
        ],
        'gradients' => [
            ...
            'tokens' => [
                ...
                'grad.primary_a' => ['label' => 'Primary from', 'type' => 'color', 'default' => '#0B4E3D'],
                'grad.primary_b' => ['label' => 'Primary to',   'type' => 'color', 'default' => '#174D3D'],
                'grad.cta_a'     => ['label' => 'CTA from',     'type' => 'color', 'default' => '#F15A24'],
                'grad.cta_b'     => ['label' => 'CTA to',       'type' => 'color', 'default' => '#E8C52E'],
        ...
        'effects' => [
            ...
            'tokens' => [
                /* Ranges clamped to the brand's radius scale (6 / 10 / 16). These
                   previously allowed 30px and 40px, which made "huge rounded cards" —
                   a thing the brand direction explicitly rules out — an admin-facing
                   setting, and pulled the shared components off the scale for the
                   whole site at once. Defaults corrected to the scale too. */
                'ui.radius'      => ['label' => 'Border radius', 'type' => 'number', 'default' => '10', 'min' => 0, 'max' => 16, 'unit' => 'px', 'var' => '--radius'],
                'ui.radius_sm'   => ['label' => 'Radius (small)','type' => 'number', 'default' => '6',  'min' => 0, 'max' => 10, 'unit' => 'px', 'var' => '--radius-sm'],
                'ui.radius_lg'   => ['label' => 'Radius (large)','type' => 'number', 'default' => '16', 'min' => 0, 'max' => 20, 'unit' => 'px', 'var' => '--radius-lg'],
        ...
        'email' => [
            ...
            'tokens' => [
                'email.header_bg' => ['label' => 'Email header colour', 'type' => 'color', 'default' => '#0B4E3D'],
                'email.btn_bg'    => ['label' => 'Email button colour', 'type' => 'color', 'default' => '#F15A24'],
                ...
                'pdf.accent'      => ['label' => 'PDF/document accent', 'type' => 'color', 'default' => '#0B4E3D'],
            ],
        ],
        'advanced' => [
            'label' => 'Advanced', 'icon' => 'code',
            'tokens' => [
                'adv.theme_color' => ['label' => 'Browser theme colour', 'type' => 'color', 'default' => '#0B4E3D'],
...
function theme_seed_presets(): void
{
    ...
    $builtin = [
        ...
        ['Forest',     'forest',     ['color.primary' => '#14532D', 'color.secondary' => '#166534', 'color.accent' => '#E8C52E', 'color.success' => '#22C55E', 'color.sidebar' => '#14532D', 'color.sidebar_2' => '#052E16', 'grad.primary_a' => '#14532D', 'grad.primary_b' => '#166534']],
        ...
        ['Sunrise',    'sunrise',    ['color.primary' => '#9A3412', 'color.secondary' => '#C2410C', 'color.accent' => '#E8C52E', 'color.sidebar' => '#7C2D12', 'color.sidebar_2' => '#431407', 'grad.primary_a' => '#9A3412', 'grad.primary_b' => '#EA580C']],
    ];
...
/**
 * The three elevation steps, tinted from the brand primary.
 *
 * $rgb is an "r,g,b" string (from theme_rgb()). It used to be hardcoded to the
 * old navy 6,53,102, so every shadow on the site — cards, dropdowns, the search
 * panel, modals — carried a blue cast no matter what the admin set the brand
 * colour to. Shadows now sit in the same hue family as the palette.
 */
function theme_shadow_set(string $depth, string $rgb = '6,53,102'): array
{
    return match ($depth) {
        'none'   => ['0 0 0 rgba(0,0,0,0)', '0 0 0 rgba(0,0,0,0)', '0 0 0 rgba(0,0,0,0)'],
        'soft'   => ["0 1px 2px rgba({$rgb},.04)", "0 2px 8px rgba({$rgb},.06)", "0 8px 20px rgba({$rgb},.10)"],
        'strong' => ["0 2px 4px rgba({$rgb},.10)", "0 8px 26px rgba({$rgb},.16)", "0 28px 60px rgba({$rgb},.26)"],
        default  => ["0 1px 2px rgba({$rgb},.07)", "0 4px 16px rgba({$rgb},.10)", "0 18px 40px rgba({$rgb},.16)"],
    };
}
...
/**
 * An AA-safe ink derived from the accent, for the places the accent itself is
 * too light: .eyebrow, .fc-label, .fb-pct, the "LIVE" chip, and every other
 * small orange label.
 *
 * WHY THIS IS DERIVED HERE AND NOT HARDCODED IN CSS
 * design-system.css sets `--accent-ink: color-mix(in srgb, var(--accent) 84%,
 * #000)` and documents "4.60:1" — which is the ratio on WHITE. Nobody checked it
 * against --bg-alt: on the alt-tinted band (#F8FCF8 in this install) the same
 * ink is 4.44:1, so EVERY .eyebrow on an alt band failed 1.4.3, site-wide, at
 * 12.5px/700 (not large text: large is >=24px, or >=18.66px bold). It is also
 * only ever correct for one accent — an admin who picks a different accent in
 * Theme Settings gets whatever 84% happens to give.
 *
 * So: pick the LIGHTEST darkening of the admin's accent that clears the target
 * on the lightest surface the ink is used on. `color-mix(in srgb, C p%, #000)`
 * is exactly per-channel scaling in sRGB, so this reproduces the CSS
 * computation in PHP and emits a plain hex — no color-mix support needed.
 *
 * Target 4.8, not 4.5: the mix is quantised to whole percents, and 82% lands on
 * 4.61 with almost no headroom, so a slightly lighter alt band would silently
 * drop under. For the shipped #F15A24 on #F8FCF8 this returns 80% = #C1481D,
 * 4.81:1 on the alt band and 4.99:1 on white (was 4.44 / 4.60).
 *
 * Darkening is safe everywhere --accent-ink is used: the other uses are
 * gradients, or a FILL under white text (design-system .btn-accent/.badge chips,
 * premium.css .icon-box.o / .pcard-badge.is-live), where white-on-it rises with
 * it — 4.99:1 at 80%. The one dark-field use is already overridden
 * (design-system.css:1179 turns .page-hero .eyebrow yellow, 7.95:1).
 */
function theme_accent_ink(array $v, float $target = 4.8): string
{
    $accent = trim((string) ($v['color.accent'] ?? '')) ?: '#F15A24';
    [$r, $g, $b] = array_map('intval', explode(',', theme_rgb($accent)));

    /* The hardest field decides. For DARK ink on a light page that is the
       DARKEST of the light surfaces, not the lightest: #F8FCF8 gives 4.72:1
       where #FFFFFF gives 4.89:1. Surfaces at or below mid luminance are
       skipped — a dark surface is not a field this ink is ever used on (the one
       dark-field label is overridden to yellow), and letting one in would
       darken the ink until it vanished. */
    $field = '#FFFFFF';
    foreach (['color.bg' => '#FFFFFF', 'color.bg_alt' => '#F1F6FB', 'color.surface' => '#FFFFFF'] as $k => $def) {
        $hex = trim((string) ($v[$k] ?? '')) ?: $def;
        $lum = theme_luminance($hex);
        if ($lum > 0.5 && $lum < theme_luminance($field)) {
            $field = $hex;
        }
    }
    ...
    $darkest = $accent;
    for ($p = 100; $p >= 30; $p--) {
        $cand = sprintf('#%02X%02X%02X',
            (int) round($r * $p / 100), (int) round($g * $p / 100), (int) round($b * $p / 100));
        $darkest = $cand;
        if (theme_contrast($cand, $field)['ratio'] >= $target) {
            return $cand;
        }
    }
    return $darkest;      // pathologically light accent — hand back the darkest tried
}
...
function theme_css_vars(string $scope = 'site', bool $withDrafts = false): string
{
    ...
    $out[] = "--grad-green:linear-gradient({$ang}deg," . ($v['color.success'] ?: '#2F8065') . ',' . ($v['color.success_dark'] ?: '#1F5C48') . ')';
    $out[] = "--grad-cta:linear-gradient({$ang}deg,{$pa}," . ($v['color.success'] ?: '#2F8065') . ')';

    [$s1, $s2, $s3] = theme_shadow_set(
        (string) ($v['ui.shadow'] ?? 'medium'),
        theme_rgb($v['color.primary'] ?: '#0B4E3D')
    );
    $out[] = '--shadow-sm:' . $s1;
    $out[] = '--shadow:' . $s2;
    $out[] = '--shadow-lg:' . $s3;
    ...
    $out[] = '--ring:rgba(' . theme_rgb($v['color.secondary'] ?: '#174D3D') . ',.35)';
    // AA-safe accent ink — see theme_accent_ink(). Emitted here rather than left
    // to design-system.css's fixed 84% mix, which fails 4.5:1 on the alt band.
    $out[] = '--accent-ink:' . theme_accent_ink($v);
...
function theme_css_rules(string $scope, bool $withDrafts = false): string
{
    ...
    $base = max(12, (int) ($v['font.base'] ?? 16));
    $css .= 'body{font-size:' . $base . 'px;line-height:' . (float) ($v['font.line'] ?? 1.65)
    ...
    /* ---------------------------------------------------------------------
       ...
       font.scale shipped as a Theme Settings control but nothing consumed it —
       no heading size derived from it, so the slider did nothing and headings
       had no systematic hierarchy. Each step is now base x ratio^n, expressed
       as a clamp() so headings stay fluid between mobile and desktop: the
       floor is one ratio step down, the ceiling is the step itself.
       --------------------------------------------------------------------- */
    $r     = max(1.05, min(1.6, (float) ($v['font.scale'] ?? 1.25)));
    $steps = ['h1' => 5, 'h2' => 4, 'h3' => 3, 'h4' => 2, 'h5' => 1, 'h6' => 0];
    $scaleCss = '';
    foreach ($steps as $tag => $n) {
        $max = $base * ($r ** $n);            // desktop ceiling
        $min = $base * ($r ** max(0, $n - 1)); // mobile floor, one step down
        $vw  = 2.2 + ($n * 0.72);              // larger headings track viewport harder
        $scaleCss .= sprintf(
            '%s{font-size:clamp(%.3frem,%.2fvw + %.3frem,%.3frem);line-height:%.3f;letter-spacing:%.3fem;}',
            $tag, $min / 16, $vw, ($min / 16) * 0.34, $max / 16,
            $n >= 4 ? 1.06 : ($n >= 2 ? 1.2 : 1.4),   // tighter leading as size grows
            $n >= 4 ? -0.025 : ($n >= 2 ? -0.015 : 0) // optical tracking correction
        );
    }
    $css .= $scaleCss;

    /* Spacing scale — one rhythm for section padding, stack gaps and gutters,
       so vertical spacing stops being ad-hoc per component. */
    $css .= ':root{--space-1:.25rem;--space-2:.5rem;--space-3:.75rem;--space-4:1rem;'
          . '--space-5:1.5rem;--space-6:2rem;--space-7:3rem;--space-8:4rem;'
          . '--space-9:6rem;--space-10:8rem;'
          . '--section-y:clamp(3.5rem,7vw,7rem);--gutter:clamp(1rem,3vw,2rem);}';
