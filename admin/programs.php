...
/**
 * =============================================================================
 *  Admin — Programs CRUD  (EXEMPLAR admin module).
 *  Demonstrates the standard pattern every admin CRUD page follows:
...
if (is_post() && post('_do') === 'save') {
    ...
    if ($errors) {
        ...
        redirect('/admin/programs?action=' . ($editId ? 'edit&id=' . $editId : 'create'));
    }
    ...
    if (!empty($_FILES['image']['name'])) {
        $up = upload_image($_FILES['image'], 'programs');
        if (!$up['success']) {
    ...
    if ($editId) {
        ...
        log_activity('update', 'programs', 'Updated program #' . $editId);
        set_flash('success', 'Program updated successfully.');
    } else {
        $newId = db_insert($table, $data);
        log_activity('create', 'programs', 'Created program #' . $newId);
...
if (is_post() && post('_do') === 'delete') {
    ...
    if ($row) {
        ...
        log_activity('delete', 'programs', 'Deleted program #' . $delId);
...
if ($action === 'create' || $action === 'edit') {
    ...
    $page_title = $action === 'edit' ? 'Edit Program' : 'Add Program';
    include __DIR__ . '/partials/head.php';
    ?>
    <div class="admin-page-head">
        <div><h1><?= e($page_title) ?></h1><span class="muted">Programs / <?= $action === 'edit' ? 'Edit' : 'Create' ?></span></div>
        <a class="btn btn-secondary" href="<?= e(admin_url('programs')) ?>">← Back to list</a>
    </div>

    <div class="panel">
        <form class="admin-form panel-body" method="post" enctype="multipart/form-data" action="<?= e(admin_url('programs')) ?>">
            <?= csrf_field() ?>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input class="form-control" name="icon" value="<?= e($row['icon'] ?? 'target') ?>" placeholder="e.g. target, heart, book">
                </div>
                <div class="form-group">
            ...
            <label class="checkbox"><input type="checkbox" name="is_featured" value="1" <?= !empty($row['is_featured']) ? 'checked' : '' ?>> Feature this program on the homepage</label>

            <div class="form-actions">
                <button class="btn btn-primary" type="submit"><?= $action === 'edit' ? 'Update' : 'Create' ?> Program</button>
                <a class="btn btn-ghost" href="<?= e(admin_url('programs')) ?>">Cancel</a>
            </div>
        </form>
    </div>
    <?php
...
?>
<div class="admin-page-head">
    <div><h1>Programs</h1><span class="muted"><?= (int) $p['total'] ?> total</span></div>
    <a class="btn btn-primary" href="<?= e(admin_url('programs?action=create')) ?>">+ Add Program</a>
</div>

<div class="panel">
    <div class="panel-body">
        <div class="data-toolbar">
            <form class="search" method="get" action="<?= e(admin_url('programs')) ?>">
                <input class="form-control" type="search" name="q" value="<?= e($search) ?>" placeholder="Search programs…">
            </form>
        </div>
        ...
        <div class="table-wrap">
            <table class="admin-table">
                <thead><tr><th>Image</th><th>Title</th><th>Status</th><th>Featured</th><th>Order</th><th style="text-align:right;">Actions</th></tr></thead>
                <tbody>
                <?php foreach ($p['items'] as $r): ?>
                    <tr>
                        ...
                        <td>
                            <div class="actions">
                                <a class="icon-btn" href="<?= e(admin_url('programs?action=edit&id=' . $r['id'])) ?>" title="Edit"><?= lucide('pencil') ?></a>
                                <form method="post" action="<?= e(admin_url('programs')) ?>" data-confirm="Delete this program permanently?" style="display:inline;">
                                    <?= csrf_field() ?>
        ...
        <?= $p['links'] ?>
        <?php else: ?>
            <div class="empty-state"><div class="icon"><?= lucide('target') ?></div>No programs yet. <a href="<?= e(admin_url('programs?action=create')) ?>">Add your first program</a>.</div>
        <?php endif; ?>
    </div>
</div>
